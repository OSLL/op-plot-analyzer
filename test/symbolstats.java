...
//					ArrayList<Integer> list = new ArrayList<Integer>();
					...
					try (CSVReader csvReader = new CSVReader(new FileReader(filename));) {
							...
							if ((temp = csvReader.readNext()) != null){		//Чтение первой строки - заголовки столбцов обрабатываться не будут
					    		String[] values = null;
					    		while (((values = csvReader.readNext()) != null) && (counter < 100024)) {
										...
										if (flag){
											...
											for (int i = 0; i<values[j].length(); i++){
												String mstr = values[j].toLowerCase();
	...
	public static String[] columnParser(String source){			//функция выделяет все составляющие строки в массив составляющих (например всех режиссеров или все страны)
